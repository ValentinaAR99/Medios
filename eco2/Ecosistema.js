...
function setup() {
  ...
  basu= new basura(random(width), random(height));
...
for (var i = 0; i < 10; i = i + 1) {
  ...
  for (var i=0; i<10; i=i +1) {
    ...
    for (var i=0; i<10; i=i +1) {
    labasura[i]= new basura(random(50,660), random (280,280));
  }
...
function draw() {
 ...
 for (var i = 0; i < numbasura; i = i + 1) {
  labasura[i].dibujarse4();
  labasura[i].moverse4();
  
    if(dist(losremys[i].posX,losremys[i].posY,labasura[i].posXba,labasura[i].posYba) <350) {
      labasura[i].morirse4();
  }
   }
  ...
  for (var i = 0; i < numremys; i = i + 1) {
    ...
    losremys[i].moverse();
    if(dist(lospaz[i].posXpal,lospaz[i].posYpal, losremys[i].posX, losremys[i].posY) <320 ){
       lospaz[i].morirse2();
    }
  ...
  lospaz[i].dibujarse2();
  lospaz[i].moverse2();
    if(dist(losremys[i].posX,losremys[i].posY,lospaz[i].posXpal,lospaz[i].posYpal) <320) {
      lospaz[i].morirse2();
  }
...
function basura (_x,_y) {
this.posXba=_x;
this.posYba=_y;
this.estaViva4=true 

this.dibujarse4= function() {
if(this.estaViva4 == true){
fill(255);
textSize(12);
textAlign(CENTER);
rect(300+this.posXba,280+this.posYba,50,50);
fill(0);
text('BASURA', 325+this.posXba, 310+this.posYba);
}
}

this.moverse4= function () {
    
    this.posXba = this.posXba + random(-0.2, 0.2)
    this.posYba = this.posYba + random(-0.2, 0.2)
}

this.morirse4 = function () {
this.estaViva4=false}
